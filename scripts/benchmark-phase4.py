#!/usr/bin/env python3
"""Run paired Phase 4 read-only benchmarks on one synthetic iCloud export."""
...
CASE_PATH = REPOSITORY_ROOT / "tests/oracle/cases/apple-photos-v3.json"
...
class Phase4BenchmarkError(RuntimeError):
    """A paired Phase 4 benchmark invariant failed."""


def _load_python(path: Path, name: str) -> ModuleType:
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise Phase4BenchmarkError(f"cannot load {path}")
...
def _command_output(command: list[str]) -> str:
    try:
        ...
        raise Phase4BenchmarkError("cannot inspect benchmark tool") from error
...
def _oracle_paths(observation: dict[str, Any], marker: str) -> set[str]:
    paths = set()
    pattern = re.compile(
        rf"{re.escape(marker)} file=icloud-\d+:(.+?)"
        r"(?:\s+(?:type|date|reason)=.*)?$"
    )
...
def _run_pair(
    ...
    def run_rust(
        ...
        command = [
            str(immich_rs), "plan", "apple-photos", "--label", "synthetic-apple",
            "--buffer-bytes", "65536", *(str(path) for path in archives),
        ]
        result, measured = metrics.run_command(command, cwd, environment, timeout)
        if result.returncode != 0:
            raise Phase4BenchmarkError("immich-rs benchmark process failed")
        try:
            plan = json.loads(result.stdout)
        except json.JSONDecodeError as error:
            raise Phase4BenchmarkError("immich-rs returned invalid JSON") from error
        if plan != expected_plan:
            raise Phase4BenchmarkError("immich-rs benchmark plan differs from its golden")
        measured["operations"] = {
    ...
    def execute_oracle(
        ...
        if len(archives) != 2:
            raise Phase4BenchmarkError("oracle benchmark did not receive two ZIP parts")
        identity = _archive_identity(archives)
        if archive_identity is not None and identity != archive_identity:
            raise Phase4BenchmarkError("materialized archive identity drifted")
    ...
    captured["immich_go"]["operations"] = {
        "source_assets_planned": len(_oracle_paths(observation, "uploaded successfully")),
...
def run(samples: int, warmups: int, oracle: Path, immich_rs: Path) -> dict[str, Any]:
    if not 2 <= samples <= 30 or not 0 <= warmups <= 4:
        raise Phase4BenchmarkError("samples must be 2..30 and warmups must be 0..4")
    if not oracle.is_file() or not immich_rs.is_file():
        raise Phase4BenchmarkError("benchmark binaries must already exist")
    revision = os.environ.get("SOURCE_REVISION") or os.environ.get("GITHUB_SHA")
    if not isinstance(revision, str) or re.fullmatch(r"[0-9a-f]{40}", revision) is None:
        raise Phase4BenchmarkError("source revision must contain the exact commit")
    runner = _load_python(REPOSITORY_ROOT / "scripts/run-oracle.py", "phase4_runner")
    metrics = _load_python(REPOSITORY_ROOT / "scripts/benchmark-metrics.py", "phase4_metrics")
    ...
    return {
        "schema": "phase4-benchmark-report-v1",
        "manifest": {
            ...
            "fixture": {
                ...
                "sidecars": expected_plan["summary"]["sidecars"],
            ...
            "methodology": {
                ...
                "pairing": "same materialized iCloud split ZIP corpus and mock environment",
...
def main() -> int:
    ...
    except (OSError, UnicodeError, ValueError, Phase4BenchmarkError) as error:
        print(f"Phase 4 benchmark failed: {error}", file=sys.stderr)
        return 1
    return 0
